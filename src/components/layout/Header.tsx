  ...
  const handleLogout = () => {
    startTransition(async () => {
      try {
        ...
        // Cookie has been deleted by the Route Handler
        router.push("/login");
        router.refresh();
      } catch (error) {
            ...
            <>
              ...
              <div className="absolute right-0 z-20 mt-2 w-56 rounded-2xl border border-slate-200 bg-white p-2 shadow-lg">
                ...
                <button
                  ...
                  disabled={isPending}
                  className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-rose-600 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {isPending ? (
                    <>
