"use client";

import { Search } from "lucide-react";

interface MasterJabatanFilterBarProps {
  searchValue: string;
  onSearchChange: (value: string) => void;
  levelOptions: string[];
  levelValue: string;
  onLevelChange: (value: string) => void;
  entityOptions: { code: string; label: string; isHo: boolean }[];
  entityValue: string;
  onEntityChange: (value: string) => void;
}

const selectClass =
  "rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-600 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20";

export function MasterJabatanFilterBar({
  searchValue,
  onSearchChange,
  levelOptions,
  levelValue,
  onLevelChange,
  entityOptions,
  entityValue,
  onEntityChange,
}: MasterJabatanFilterBarProps) {
  const hoOptions = entityOptions.filter((e) => e.isHo);
  const regionalOptions = entityOptions.filter((e) => !e.isHo);

  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
      <div className="relative flex-1">
        <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
        <input
          type="text"
          value={searchValue}
          onChange={(e) => onSearchChange(e.target.value)}
          placeholder="Cari nama atau kode jabatan..."
          className="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-9 pr-3 text-sm text-slate-700 placeholder:text-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
        />
      </div>

      <select
        value={levelValue}
        onChange={(e) => onLevelChange(e.target.value)}
        className={selectClass}
      >
        <option value="all">Semua Level</option>
        {levelOptions.map((level) => (
          <option key={level} value={level}>
            {level}
          </option>
        ))}
      </select>

      <select
        value={entityValue}
        onChange={(e) => onEntityChange(e.target.value)}
        className={selectClass}
      >
        <option value="all">Semua Entity</option>
        {hoOptions.length > 0 && (
          <optgroup label="Head Office">
            {hoOptions.map((e) => (
              <option key={e.code} value={e.code}>
                {e.label}
              </option>
            ))}
          </optgroup>
        )}
        {regionalOptions.length > 0 && (
          <optgroup label="Regional">
            {regionalOptions.map((e) => (
              <option key={e.code} value={e.code}>
                {e.label}
              </option>
            ))}
          </optgroup>
        )}
      </select>
    </div>
  );
}
